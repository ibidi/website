    ...
    <div class="container mx-auto max-w-4xl">
      ...
      <div v-if="posts && posts.length" class="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
        <div 
          v-for="post in posts" 
          :key="post._path"
          class="bg-zinc-50 dark:bg-zinc-800/50 p-6 rounded-lg shadow-md hover:shadow-xl transition-shadow duration-300 ease-in-out"
        >
          <NuxtLink :to="post._path" class="block">
            <h3 class="text-xl font-semibold text-zinc-800 dark:text-zinc-100 mb-2 hover:text-violet-500 dark:hover:text-violet-400 transition-colors">
              {{ post.title }}
            </h3>
          </NuxtLink>
          <p v-if="post.description" class="text-zinc-600 dark:text-zinc-400 text-sm mb-3 leading-relaxed">
            {{ post.description }}
          </p>
          ...
          <NuxtLink :to="post._path" class="inline-block mt-4 text-sm text-violet-600 dark:text-violet-400 hover:underline">
            Read more &rarr;
          </NuxtLink>
        </div>
      </div>
